...
SELECT h.id AS horse_id,
       ...
       h.status
FROM horses h
LEFT JOIN breeds b ON b.id = h.idbreed
WHERE h.status = 'active'
  AND h.id NOT IN (
      SELECT wp.idhorse
      FROM workouts w
      JOIN workout_participants wp ON wp.idworkout = w.id
...
LEFT JOIN (
    ...
    FROM workout_participants wp
    LEFT JOIN workouts w ON w.id = wp.idworkout
    GROUP BY wp.idrider
) stat ON stat.idrider = r.id;

-- Пример использования:
-- SELECT * FROM v_riders_balance WHERE subscription_balance > 0 ORDER BY rider_full_name;
...
SELECT DATE_TRUNC('month', w.datetime_start)::date AS period_month,
...
-- Пример использования:
-- SELECT * FROM v_revenue_summary WHERE period_month >= DATE '2025-01-01';
...
DECLARE
    total NUMERIC;
BEGIN
    SELECT COALESCE(SUM(s.price), 0) INTO total
    FROM workouts w
    LEFT JOIN services s ON s.id = w.idservice
    WHERE w.status = 'done'
...
-- 9. Хранимая функция для расчёта количества проведённых тренировок тренера за период
CREATE OR REPLACE FUNCTION fn_trainer_workouts_count(trainer_id INTEGER, d_from DATE, d_to DATE)
RETURNS INTEGER AS $$
DECLARE
    cnt INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION fn_horse_available(horse_id INTEGER, at_time TIMESTAMP)
RETURNS BOOLEAN AS $$
DECLARE
    busy_count INTEGER;
...
DECLARE
    rider RECORD;
BEGIN
    UPDATE workouts SET status = 'done' WHERE id = workout_id;

    FOR rider IN
        SELECT idrider FROM workout_participants
        WHERE idworkout = workout_id AND idrider IS NOT NULL
    LOOP
...
-- Пример вызова:
-- CALL sp_add_rider('Иван', 'Петров', DATE '2005-04-12',
...
BEGIN
    ...
    IF overlap_count > 0 THEN
        RAISE EXCEPTION 'У тренера id=% уже есть тренировка на %',
...
FOR EACH ROW EXECUTE FUNCTION trg_check_workout_overlap();
...
FOR EACH ROW EXECUTE FUNCTION trg_users_username_lower();
